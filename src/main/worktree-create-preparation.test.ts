...
describe('worktree create preparation registry', () => {
  ...
  it('retries only the leaked paths belonging to the host being prepared', async () => {
    ...
    // Give each host one recorded discard failure. Eviction prefers the incoming workspace's own
    // oldest entry, so a repo-1 burst can no longer evict repo-2's preparation for us.
  ...
  it('scopes retries to the WSL distro whose preparation leaked', async () => {
    ...
    // Give each distro one recorded discard failure. Eviction prefers the incoming host's own
    // oldest entry, so an Ubuntu burst can no longer evict the Debian preparation for us.
    for (const base of ['origin/one', 'origin/two']) {
      await prepareWorktreeCreateForRepo(store, repo, base)
    }
    mocks.getWorktreeOptions.mockReturnValue({ wslDistro: 'Ubuntu' })
